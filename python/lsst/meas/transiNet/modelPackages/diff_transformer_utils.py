...
class LocalAttention(nn.Module):
    def __init__(self, h, w, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, device=None):
        ...
        #self.alpha = nn.Parameter(torch.zeros(1, 1, h * w, h * w, device=device))
        self.alpha = nn.Parameter(torch.zeros(1, 1, 1, 1, device=device))
        with torch.no_grad():
            pos = pos_meshgrid(h, w).reshape(h * w, 2).unsqueeze(0)
            pos_ = pos_meshgrid(h, w).reshape(h * w, 2).unsqueeze(0)
            dist = torch.cdist(pos, pos_).unsqueeze(0) # (1, 1, hw, hw)
        if device:
            dist = dist.to(device)
        self.dist = dist
        self.apply(self._init_weights)
...
class AttentionDecoderBlock(nn.Module):
    ...
    def __init__(self, h, w, in_channels, pos_dim=16, n_heads=4, attn_drop=0., drop=0., mlp_ratio=4., qkv_bias=True):
        ...
        self.src_mlp = Mlp(in_features=self.out_dim, hidden_features=mlp_hidden_dim, act_layer=nn.GELU, drop=drop)
        ...
        mlp_hidden_dim = int(self.out_dim * mlp_ratio)
        self.comb_mlp = Mlp(in_features=self.out_dim, hidden_features=mlp_hidden_dim, act_layer=nn.GELU, drop=drop)

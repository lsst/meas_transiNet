__all__ = ["TripletDifferenceTransformerV8"]
import os
import math
import torch
import torch.nn as nn
import torch.nn.functional as F
from diff_transformer_utils import ImageEmbedder, RelativePositionAttention, Mlp, resize, conv_diff, make_prediction, AttentionDecoderBlock, TripletAttentionDecoderBlock
...
class TripletDifferenceTransformerV8(nn.Module):
    ...
    def __init__(self, img_size, in_channels=1, embed_dim=256, device=None, depths=[3, 3, 3, 3, 3, 3],
        ...
        self.hs = self.enc.hs
        self.ws = self.enc.ws
        self.dec = Decoder(in_index=list(range(n)), in_channels=self.embed_dims, hs=self.hs, ws=self.ws)
...
class EncoderTransformer(nn.Module):
    ...
    def __init__(self, img_size, kernel_size=3, depths=[3, 3, 6, 18], embed_dims=[32, 64, 128, 256],
        ...
        self.img_embedders = nn.ModuleList([
            ImageEmbedder(self.img_size, kernel_size=kernel_sizes[i], stride=strides[i],
    ...
    def forward(self, src, temp, diff):
        ...
        x3 = diff
        for i in range(self.n):
            x1, H1, W1 = self.img_embedders[i](x1)
            x2, H2, W2 = self.img_embedders[i](x2)
            x3, H3, W3 = self.img_embedders[i](x3)
...
class Decoder(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], in_channels=[64, 128, 256, 512],
        ...
        for i in range(n):
            ...
            self.diffs.append(conv_diff(in_channels=in_channels[i]+pos_dim, out_channels=self.embedding_dim))
            self.make_preds.append(make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc))

        self.attn_decoders = nn.ModuleList(self.attn_decoders)
        self.diffs = nn.ModuleList(self.diffs)
        self.make_preds = nn.ModuleList(self.make_preds)
    ...
    def forward(self, x_1, x_2, x_3):
        ...
        for i in range(len(x_1)):
            ...
            c = self.attn_decoders[i](x_1[i], x_2[i], x_3[i]).permute(0,2,1).reshape(n, -1, self.hs[i], self.ws[i])
            c = self.diffs[i](c)
            c = self.make_preds[i](c)
            outputs.append(c)

        outputs = [torch.flatten(x, start_dim=1) for x in outputs]
        out = torch.cat(outputs, dim=1)
        out = torch.relu(self.lin_1(out))
        out = torch.sigmoid(self.lin_2(out)).squeeze(1)
        return out
